"use client";
import { EditorLibraryPanel } from "@/components/editor/editor-library-panel";
...
import { capturePostHogEvent, trackEvent } from "@/lib/tracking";
...
export const MeshExports = ({ contentRef, savedGradient, viewer }: Props) => {
	...
	const currentWidth = contentRef?.current?.clientWidth ?? canvas.width;
	const currentHeight = contentRef?.current?.clientHeight ?? canvas.height;
	const currentShareState = toShareString();
		...
		<div className="absolute top-0 right-0 p-1 rounded-lg bg-bg z-50 shadow">
			<Popover>
				...
				<Popover.Content className="sm:min-w-72">
					...
					<Separator />
					<Popover.Body className="flex flex-col gap-2 p-3 max-h-[calc(100vh-130px)] overflow-y-auto">
